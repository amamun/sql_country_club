...
SELECT name AS 'name of the facility'
FROM Facilities
WHERE membercost > 0;

/* Q2: How many facilities do not charge a fee to members? */
SELECT COUNT(name) AS 'name of the facility'
FROM Facilities
WHERE membercost > 0;
...
SELECT 	name, montlymaintenance,
	(CASE WHEN montlymaintenance < 100 THEN 'cheap' 
	      ELSE 'expensive' END) AS expensiveness
FROM	Facilities;	
...
WHERE 	b.starttime LIKE '2012-09-14%' 
	AND ((m.memid = 0 AND f.guestcost * b.slots > 30)
	OR (m.memid != 0 AND f.membercost * b.slots > 30))) AS Cost_30
ORDER BY cost DESC;
